...
export const GET: RequestHandler = async () => {
  try {
    console.log('=== freee Wallet Transactions API Test ===');
    ...
    console.log('Wallet Txns retrieved:', walletTxns.length);
    if (walletTxns.length > 0) {
      console.log('Sample wallet transaction:', JSON.stringify(walletTxns[0], null, 2));
    }
    
    return json({
      success: true,
      count: walletTxns.length,
      sample: walletTxns.length > 0 ? {
        id: walletTxns[0].id,
        date: walletTxns[0].date,
        partner_name: walletTxns[0].partner_name,
        description: walletTxns[0].description,
        amount: walletTxns[0].amount
      } : null,
      // 詳細構造の確認
      rawData: walletTxns.length > 0 ? walletTxns[0] : null,
      allFields: walletTxns.length > 0 ? Object.keys(walletTxns[0]) : [],
      // 最初の数件のデータも確認
